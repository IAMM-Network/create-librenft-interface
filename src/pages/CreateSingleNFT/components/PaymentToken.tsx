...
const PaymentTokens = (props: PaymentTokenProps) => {
  const handleSalePrice = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value != null && value.length) {
      ...
      let price = BigNumber.from(value).mul(BigNumber.from(10).pow(parseInt(decimals.value)))
    ...
    <>
      <Section>
        <Flex flexDirection='column'>
          ...
          <Input id='token-decimals' placeholder='Enter token decimals or leave 18 for default' defaultValue={18} />
          <Text margin='0.5rem 0 0 0'>Enter NFT sale price</Text>

          <Input placeholder='Enter NFT sale price' defaultValue={props.nftConfig.price} onChange={handleSalePrice} />
        </Flex>
      </Section>
    </>
  )
